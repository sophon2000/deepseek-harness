...
Parameter names in Remote Client declarations come from wire fields, while parameter and return types reference Client-safe symbols exposed through concrete public non-root type subpaths. The symbol owner may be the contributing business package or an installed NPM dependency; installed declarations remain external to the workspace type graph. The declaration map resolves the generated property behind `ctx.remote.goals.create` back to the Host source method marked with `@Remote`, so editors that support declaration maps can navigate from a Client call to the real implementation instead of stopping at the generated `.d.ts`.
