...
`Workspace.sessionIds` 有意作为已校验的成员投影：只有当会话 id 位于持久候选记账中，并且其索引中的规范 header cwd 等于 Workspace 路径时，该会话才会出现。缺失 header、cwd 不可用或 cwd 不匹配的会话必须留在 GUI 分组与 Workspace 继承之外。[Workspace UI 产品流程决策](../../../../packages/workspace/workspace/README.zh.md)继续拥有这项成员资格与分组约定。
