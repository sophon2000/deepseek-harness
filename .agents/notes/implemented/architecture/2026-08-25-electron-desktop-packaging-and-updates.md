...
A product release may additionally carry one signed background-service declaration. The declaration contains only a portable entrypoint below the product's inventoried resource root and an exact list of environment names it may return. Electron starts that entrypoint under bundled Node.js before the Host, rejects traversal, process-control environment names, missing or extra ready values, and stores product-owned mutable data below `.dsh/desktop/products/<id>`. Shutdown is dependency ordered: Host first, product service second. The generic shell knows no product commands, ports, database schema, or credentials.
...
| Signed product service | Product-local API/database lifecycle and durable product data; returns only its declared Host environment over ready IPC |
...
`dev:desktop` builds the current workspace, projects the built CLI and private Desktop Host packages plus their dependency links into a disposable project, uses an isolated Harness home, opens the Main, Renderer, and Host debuggers, and starts unpackaged Electron without preparing release resources. A product repository may explicitly add already-built, uniquely named package directories and system-trusted preset roots to this disposable project; declared bundles are enabled automatically. Only the linked development Host accepts those out-of-tree roots, while packaged and otherwise managed profiles reject them. Package mutation remains disabled because the linked dependency graph is not a pnpm-installed desktop project. Fixed macOS arm64, macOS x64, and Windows x64 package commands pass one target through runtime preparation, dsh preparation, and electron-builder; each also has an unpacked-directory variant for release-path verification before installer generation.
