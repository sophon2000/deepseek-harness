...
A product release may additionally carry one signed background-service declaration. The declaration contains only a portable entrypoint below the product's inventoried resource root and an exact list of environment names it may return. Electron starts that entrypoint under RunAsNode before the Host, rejects traversal, process-control environment names, missing or extra ready values, and stores product-owned mutable data below `.dsh/desktop/products/<id>`. Shutdown is dependency ordered: Host first, product service second. The generic shell knows no product commands, ports, database schema, or credentials.
