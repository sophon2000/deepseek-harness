...
The verification also packs the Landlock entry, which `dsh-sandbox-local` declares as a plain dependency. It keeps optional dependencies enabled and lets npm skip only those that are incompatible with the host: external packages such as Koffi distribute their host prebuild through `optionalDependencies`, so globally omitting optional dependencies turns an ordinary install into an accidental native source build. The Landlock platform packages still need a musl toolchain and one build per architecture, so the verification does not require one runner to produce every platform package. It reads a directory by its contents rather than a pack order, because a directory can hold tarballs packed only to satisfy a cross-sequence dependency.
