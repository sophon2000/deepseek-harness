...
| Product services | A product may require a local API or database before its plugins can start, but business-specific process knowledge does not belong in the generic shell. | A signed product descriptor may name one entrypoint below its inventoried resource root and an exact environment allowlist. Electron starts it under Electron RunAsNode, waits for ready IPC, starts the Host with only those returned values, and stops the Host before the product service. |
...
Product repositories may extend only this disposable development profile with repeatable `--profile-package` and `--system-preset-root` arguments. Declared bundle packages are enabled automatically; only the linked development Host accepts those out-of-tree preset roots.
